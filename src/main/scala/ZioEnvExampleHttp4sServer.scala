...
object ZioEnvExampleHttp4sServer extends App {
  ...
  object PetLayer {
    ...
    object PetService {
      ...
      val live: ZLayer[Console, String, PetService] = ZLayer.fromFunction {
        console: Console => petId: Int =>
          console.get
            .putStrLn(s"Got request for pet: $petId")
  ...
  val petRoutes: HttpRoutes[RIO[PetService with Clock, *]] =
    ZHttp4sServerInterpreter
      .from(petEndpoint)(petId => PetService.find(petId))
      .toRoutes

  // Same as above, but combining endpoint description with server logic:
  val petServerEndpoint: ZServerEndpoint[PetService, Int, String, Pet] =
  ...
  val yaml: String = {
    ...
    OpenAPIDocsInterpreter
      .toOpenAPI(List(petEndpoint), "Our pets", "1.0")
      .toYaml
  }

  // Starting the server
  val serve: ZIO[ZEnv with PetService, Throwable, Unit] =
    ZIO.runtime[ZEnv with PetService].flatMap { implicit runtime =>
